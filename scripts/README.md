...
We've created a convenient Python script to start Llama Stack:
...
chmod +x scripts/start_llama_stack.py
...
2. Syncs dependencies using `uv sync` (creates `.venv` if needed)
3. Verifies Ollama installation and service
4. Checks for required models and pulls them if missing
5. Sets environment variables
6. Starts Llama Stack server
...
### Issue: Missing module errors (together, faiss, etc.)

**Solution:**
```bash
# Install all required provider dependencies
uv sync
...
**Note:** Llama Stack's starter distribution initializes all providers, so all optional provider dependencies must be installed even if you only use Ollama.
...
The `start_llama_stack.py` script automates the entire setup and startup process:
...
The script will:
- Check prerequisites (uv, Ollama, Python)
- Sync dependencies
